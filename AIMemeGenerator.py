#!/usr/bin/env python3
# AI Image Generator
# Creates start-to-finish images using various AI service APIs. OpenAI's chatGPT to generate the image text and image prompt, and several optional image generators for the image picture. Then combines the image text and image into a image using Pillow.
# Author: ThioJoe
# Project Page: https://github.com/ParaliyzedEvo/Full-Stack-AI-Image-Generator
version = "1.0.0"
...
def construct_system_prompt(basic_instructions, image_special_instructions):
    format_instructions = f'You are a image generator with the following formatting instructions. The user will send you a message with a general theme or concept on which you will base the image. The user may choose to send you a text saying something like "anything" or "whatever you want", or even no text at all, which you should not take literally, but take to mean they wish for you to come up with something yourself. In any case, you will respond with two things: First, the text of the image that the user has typed to you and will be the theme of the final image. Second, some text that will be used as an image prompt for an AI image generator to generate an image to also be used as part of the image. You must respond only in the format as described next, because your response will be parsed, so it is important it conforms to the format. The first line of your response should be: "Image Text: " followed by the Image text. The second line of your response should be: "Image Prompt: " followed by the image prompt text.  --- Now here are additional instructions... '
    basicInstructionAppend = f'Next are instructions for the overall approach you should take to creating the image. Interpret as best as possible: {basic_instructions} | '
...
parser.add_argument("--userprompt", help="A image subject or concept to send to the chat bot. If not specified, the user will be prompted to enter a subject or concept.")
parser.add_argument("--imagecount", help="The number of images to create. If using arguments and not specified, the default is 1.")
...
parser.add_argument("--nofilesave", action='store_true', help="If specified, the image will not be saved to a file, and only returned as virtual file part of imageResultsDictsList.")
...
def validate_api_keys(apiKeys, image_platform):
    if not apiKeys.openai_key:
        print("\n  ERROR:  No OpenAI API key found. OpenAI API key is required - In order to generate text for the image text and image prompt. Please add your OpenAI API key to the api_keys.ini file.")
        input("\nPress Enter to exit...")
        sys.exit()
...
# Write or append log file containing the user user message, chat bot image text, and chat bot image prompt for each image
def write_log_file(userPrompt, AiImageDict, filePath, logFolder, basic, special, platform):
    ...
    with open(os.path.join(logFolder, "log.txt"), "a", encoding='utf-8') as log_file:
        log_file.write(textwrap.dedent(f"""
                       ...
                       Chat Bot Image Text: {AiImageDict['image_text']}
                       Chat Bot Image Prompt: {AiImageDict['image_prompt']}
...
def check_for_update(currentVersion=version, updateReleaseChannel=None, silentCheck=False):
    ...
    try:
        if updateReleaseChannel.lower() == "stable":
            response = requests.get("https://api.github.com/repos/ParaliyedEvo/Full-Stack-AI-Image-Generator/releases/latest")
        ...
        if response.status_code != 200:
            if response.status_code == 403:
                ...
                if silentCheck == False:
                    ...
                    print(f"If this keeps happening, you may want to report the issue here: https://github.com/ParaliyzedEvo/Full-Stack-AI-Image-Generator/issues")
                else:
                    print(f"Error [U-3]: Got non 200 status code (got: {response.status_code}) when attempting to check for update.\n")
                return None
        ...
        if silentCheck == False:
            ...
            print("If this keeps happening, you may want to report the issue here: https://github.com/ParaliyzedEvo/Full-Stack-AI-Image-Generator/issues")
    ...
    if parse_version(latestVersion) > parse_version(currentVersion):
        ...
        if silentCheck == False:
            ...
            if isBeta == True:
                print(f" A new beta version is available! To see what's new visit: https://github.com/ParaliyzedEvo/Full-Stack-AI-Image-Generator/releases ")
            else:
                print(f" A new version is available! To see what's new visit: https://github.com/ParaliyzedEvo/Full-Stack-AI-Image-Generator/releases ")
...
# Gets the image text and image prompt from the message sent by the chat bot
def parse_image(message):
    ...
    if match:
        # If image text is enclosed in quotes it will be in group 2, otherwise, it will be in group 3.
        image_text = match.group(2) if match.group(2) is not None else match.group(3)
...
def send_and_receive_message(text_model, userMessage, conversationTemp, temperature=0.5):
    ...
    print("Sending request to write image...")
...
def create_image(image_path, top_text, filePath, fontFile, noFileSave=False, min_scale=0.05, buffer_scale=0.03, font_scale=1):
    ...
    virtualImageFile = io.BytesIO()
    new_img.save(virtualImageFile, format="PNG")
    
    return virtualImageFile
...
def generate(
    text_model="gpt-4",
    temperature=1.0,
    basic_instructions=r'You will create image that are good and original, and not cliche or lame.',
    ...
    # ---------- Start User Input -----------
    # Display Header
    print(f"\n==================== AI Image Generator - {version} ====================")

    if noUserInput:
        userEnteredPrompt = user_entered_prompt
        image_count = image_count # Use default set in function parameter (1)
    ...
    # If any arguments are being used (or set to true for store_true arguments), skip the user input and use the arguments or defaults
        ...
        if not args.userprompt:
            print("\nEnter a image subject or concept (Or just hit enter to let the AI decide)")
        ...
        # If no image count argument set, get user input for image count
        if not args.imagecount:
            # Set the number of images to create
            image_count = 1 # Default will be none if nothing entered
            print("\nEnter the number of images to create (Or just hit Enter for 1): ")
            userEnteredCount = input(" >  ")
            if userEnteredCount:
                image_count = int(userEnteredCount)
        else:
            image_count = int(args.imagecount)


    def single_image_generation_loop():
        # Send request to chat bot to generate image text and image prompt
        chatResponse = send_and_receive_message(text_model, userEnteredPrompt, conversation, temperature)

        # Take chat message and convert to dictionary with image_text and image_prompt
        imageDict = parse_image(chatResponse)
        image_prompt = imageDict['image_prompt']
        image_text = imageDict['image_text']

        # Print the image text and image prompt
        ...
        # Combine the image text and image into a image
        filePath,fileName = set_file_path(base_file_name, output_folder)
        virtualImageFile = create_image(virtual_image_file, image_text, filePath, noFileSave=noFileSave,fontFile=font_file)
        if not noFileSave:
            # Write the user message, image text, and image prompt to a log file
            write_log_file(userEnteredPrompt, imageDict, filePath, output_folder, basic_instructions, image_special_instructions, image_platform)
        
        absoluteFilePath = os.path.abspath(filePath)
        
        return {"image_text": image_text, "image_prompt": image_prompt, "file_path": absoluteFilePath, "virtual_image_file": virtualImageFile, "file_name": fileName}

    # Create list of dictionaries to hold the results of each image so that they can be returned by main() if called from command line
    imageResultsDictsList = []

    for i in range(image_count):
        print("\n----------------------------------------------------------------------------------------------------")
        print(f"Generating image {i+1} of {image_count}...")
        imageInfoDict = single_image_generation_loop()

        # Add image info dict to list of image results
        imageResultsDictsList.append(imageInfoDict)
    
    # If called from command line, will return the list of image results
    return imageResultsDictsList
