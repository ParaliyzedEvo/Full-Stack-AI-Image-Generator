#!/usr/bin/env python3
# AI Image Generator
# Creates start-to-finish image using various AI service APIs. OpenAI's chatGPT to generate the image text and image prompt, and several optional image generators for the image picture. Then creates an image using Pillow.
# Author: Paraliyzed_evo
...
class MissingOpenAIKeyError(Exception):
    def __init__(self, message):
        full_error_message = f"No OpenAI API key found. OpenAI API key is required - In order to generate text for the image text and image prompt. Please add your OpenAI API key to the api_keys.ini file."
...
def construct_system_prompt(basic_instructions, image_special_instructions):
    format_instructions = f'You are a image generator with the following formatting instructions. The user will send you a message with a general theme or concept on which you will base the image. The user may choose to send you a text saying something like "anything" or "whatever you want", or even no text at all, which you should not take literally, but take to mean they wish for you to come up with something yourself. You must respond only in the format as described next, because your response will be parsed, so it is important it conforms to the format. The first and only line of your response should be: "Image Prompt: " --- Now here are additional instructions... '
    basicInstructionAppend = f'Next are instructions for the overall approach you should take to creating the images. Interpret as best as possible: {basic_instructions} | '
...
def check_for_update(currentVersion=version, updateReleaseChannel=None, silentCheck=False):
    ...
    try:
        if updateReleaseChannel.lower() == "stable":
            response = requests.get("https://api.github.com/repos/ParaliyzedEvo/Full-Stack-AI-Image-Generator/releases/latest")
        elif updateReleaseChannel.lower() == "all":
            response = requests.get("https://api.github.com/repos/ParaliyzedEvo/Full-Stack-AI-Image-Generator/releases")
...
def send_and_receive_message(text_model, userMessage, conversationTemp, temperature=0.5):
    ...
    print("Sending request to create image...")
...
def generate(
    text_model="gpt-4",
    temperature=1.0,
    basic_instructions=r'You will create good and creative images that are clever and original, and not cliche or lame.',
    ...
    def single_image_generation_loop():
        ...
        if not noFileSave:
            # Write the user message image prompt to a log file
        ...
        return {"image_prompt": image_prompt, "file_path": absoluteFilePath, "virtual_image_file": virtualImageFile, "file_name": fileName}
    ...
    try:
        ...
        print(f"\n  ERROR:  An error occurred while generating the image. Error: {ex}")
        if not noUserInput:
            input("\nPress Enter to exit...")
        sys.exit()
